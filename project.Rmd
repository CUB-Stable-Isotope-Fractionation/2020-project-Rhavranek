...
```
### Activity 3: Using the vapor transport df and liquid transport df, make a plot of depth vs. d18O

```{r}
d18O_profile <- bind_rows(vt_region, lt_region) %>% 
  select(z, d18O)

Isothermal_unsaturated <- d18O_profile %>% 
    ggplot() + 
    aes(d18O, z)+
    geom_path() + 
    scale_y_reverse()
...
Isothermal_unsaturated
```

What is going on at the top of the profile?
...
vt_ni <-  tibble(
  ...
  diffusion_term  = (eta*(1+d18O_res)) + (d18O_res - d18O_a),
  humidity_term = (z/ (z+h_a*z_bar)),
  d18O_summer =  (d18O_a + epsilon_d18O_eq + diffusion_term*humidity_term)/alpha_eq
...
lt_ni <- tibble (
  ...
  #equation 6 from 1988 paper: 
  d18O_summer = d18O_res + (3.74 - d18O_res)*exp(-1/k*(z-z_ef))
)

ni_profile <- bind_rows(vt_ni,lt_ni) %>% select (z, d18O_summer) 
```


```{r}
d18O_profile_compare <- bind_cols(d18O_profile, ni_profile) %>% 
  pivot_longer(c(d18O, d18O_summer)) %>%   
    ggplot()+
